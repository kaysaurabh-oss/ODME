...
# =============================================================================
# Manual Superbrain market-view layer (V1)
# =============================================================================


def _dir_from_odme_tilt(tilt: str) -> str:
    t = str(tilt or '').upper()
    if 'BULLISH' in t:
        return 'BULLISH'
    if 'BEARISH' in t:
...
def _pitchfork_reversal_support(direction: str, position: str, reclaimed_2sd: bool) -> bool:
    p = str(position or '')
    if direction == 'BULLISH':
        if p == 'Lower 1SD to 2SD':
            return True
        if p == 'Below lower 2SD':
            return bool(reclaimed_2sd)
        return False
    if p == 'Upper 1SD to 2SD':
        return True
    if p == 'Above upper 2SD':
        return bool(reclaimed_2sd)
    return False


def superbrain_market_view(
    *,
    location_mode: str,
    macro: str,
    odme_tilt: str,
    exec_of: str,
    aurora: str,
    pitchfork_position: str = 'Near median',
    reclaimed_2sd: bool = False,
) -> Dict[str, str]:
    odme = _dir_from_odme_tilt(odme_tilt)
    of_dir = str(exec_of or 'NEUTRAL').upper()
    macro_dir = str(macro or 'NEUTRAL').upper()
    aurora_dir = _aurora_dir(aurora)

    if location_mode == "No man's land":
        if odme == 'NEUTRAL':
            return {
                'mode': 'OPEN WATER',
                'decision': 'NO TRADE',
                'direction': 'NEUTRAL',
                'reason': 'ODME has no clean directional edge. Open-water trades need ODME and Exec OF agreement.',
                'next': 'Wait for a directional ODME read and matching Exec OF.',
            }
        if of_dir == 'NEUTRAL':
            return {
                'mode': 'OPEN WATER',
                'decision': 'WAIT',
                'direction': odme,
                'reason': f'ODME is {odme.lower()} but Exec OF is neutral.',
                'next': f'Wait for Exec OF to turn {odme.lower()}, then require matching AURORA.',
            }
        if odme != of_dir:
            return {
                'mode': 'OPEN WATER',
                'decision': 'NO TRADE',
                'direction': 'CONFLICT',
                'reason': f'ODME is {odme.lower()} while Exec OF is {of_dir.lower()}. No location edge exists to justify a reversal.',
                'next': 'Wait for ODME and Exec OF to align.',
            }
        if aurora_dir != odme:
            wanted = 'Green' if odme == 'BULLISH' else 'Red'
            return {
                'mode': 'OPEN WATER',
                'decision': 'WAIT',
                'direction': odme,
                'reason': f'ODME and Exec OF agree {odme.lower()}, but AURORA is {aurora}.',
                'next': f'Wait for AURORA {wanted}.',
            }
        side = 'LONG' if odme == 'BULLISH' else 'SHORT'
        return {
            'mode': 'OPEN WATER',
            'decision': f'{side} CONTINUATION ELIGIBLE',
            'direction': odme,
            'reason': f'ODME and Exec OF agree {odme.lower()} and AURORA confirms the same direction.',
            'next': 'Continuation conditions are satisfied.',
        }

    intended = 'BULLISH' if location_mode == 'Bullish POI' else 'BEARISH'
    opposite = _opposite(intended)
    votes = sum(x == intended for x in [macro_dir, odme, of_dir])
    side = 'LONG' if intended == 'BULLISH' else 'SHORT'
    wanted_aurora = 'Green' if intended == 'BULLISH' else 'Red'

    # ODME has higher directional authority in the agreed framework.
    if odme == opposite and not (odme == of_dir == intended):
        return {
            'mode': 'POI',
            'decision': 'WAIT',
            'direction': intended,
            'reason': f'Qualified {location_mode.lower()} is active, but ODME is explicitly {odme.lower()} against it.',
            'next': 'Wait for ODME to stop opposing the POI thesis.',
        }

    # Normal POI confirmation: two directional authorities agree and OF is not opposing.
    if votes >= 2 and of_dir != opposite:
        if aurora_dir == intended:
            return {
                'mode': 'POI',
                'decision': f'{side} ELIGIBLE',
                'direction': intended,
                'reason': f'Qualified {location_mode.lower()}, directional permission is present, and AURORA {wanted_aurora} confirms execution.',
                'next': 'Market-view conditions are confirmed.',
            }
        return {
            'mode': 'POI',
            'decision': 'WAIT',
            'direction': intended,
            'reason': f'Qualified {location_mode.lower()} and directional permission are present, but AURORA has not confirmed {intended.lower()} transfer.',
            'next': f'Wait for AURORA {wanted_aurora}.',
        }

    # Early reversal exception: OF still opposes, but the POI has structural stretch and AURORA reverses.
    if of_dir == opposite:
        structural_support = _pitchfork_reversal_support(intended, pitchfork_position, reclaimed_2sd)
        strategic_support = macro_dir == intended or odme == intended
        if strategic_support and odme != opposite and structural_support and aurora_dir == intended:
            return {
                'mode': 'POI',
                'decision': f'EARLY {side} REVERSAL ELIGIBLE',
                'direction': intended,
                'reason': f'Exec OF still opposes, but the qualified POI has Pitchfork reversal stretch and AURORA has transferred {intended.lower()}.',
                'next': f'Exec OF turning {intended.lower()} would upgrade confirmation.',
            }
        missing = []
        if not strategic_support:
            missing.append('Macro/ODME support')
        if not structural_support:
            missing.append('Pitchfork stretch/reclaim')
        if aurora_dir != intended:
            missing.append(f'AURORA {wanted_aurora}')
        return {
            'mode': 'POI',
            'decision': 'WAIT',
            'direction': intended,
            'reason': 'POI is active but Exec OF still opposes. Early reversal conditions are incomplete.',
            'next': 'Need ' + ', '.join(missing) + '.' if missing else 'Wait for reversal confirmation.',
        }
    ...
    return {
        ...
        'decision': 'WAIT',
        'direction': intended,
        'reason': 'Qualified POI is active, but directional permission is not yet sufficient.',
        'next': 'Wait for Macro/ODME/Exec OF alignment or a valid Pitchfork + AURORA reversal sequence.',
    }


def render_superbrain_manual(display: Dict[str, Any], instrument: str) -> None:
    st.markdown('### Superbrain — Manual Market View')
    st.caption('V1 uses the existing ODME read automatically. Enter only the TradingView states manually.')

    odme_tilt = display.get('tilt', 'MIXED / NO CLEAN EDGE')
    odme_dir = _dir_from_odme_tilt(odme_tilt)
    c1, c2, c3 = st.columns(3)
    with c1:
        render_card('Instrument', instrument, 'Selected ODME instrument', 'tint-grey')
    with c2:
        render_card('ODME', odme_dir, odme_tilt, _tint_for_tilt(odme_tilt))
    with c3:
        render_card('Input Mode', 'MANUAL TV STATE', 'Replace with TV alerts later', 'tint-blue')

    location_mode = st.selectbox(
        '1. Current location',
        ["No man's land", 'Bullish POI', 'Bearish POI'],
        key=f'sb_location_{instrument}',
        help='POI means price is interacting with a qualified EDGE + Footprint location.',
    )

    cols = st.columns(3)
    with cols[0]:
        exec_of = st.selectbox('2. Exec-TF OF Control', ['BULLISH', 'NEUTRAL', 'BEARISH'], key=f'sb_of_{instrument}')
    with cols[1]:
        aurora = st.selectbox('3. AURORA', ['GREEN', 'YELLOW', 'WHITE', 'PINK', 'RED'], key=f'sb_aurora_{instrument}')
    with cols[2]:
        if location_mode == "No man's land":
            macro = 'NEUTRAL'
            st.selectbox('4. Macro', ['Not used in open water'], disabled=True, key=f'sb_macro_disabled_{instrument}')
        else:
            macro = st.selectbox('4. Macro', ['BULLISH', 'NEUTRAL', 'BEARISH'], key=f'sb_macro_{instrument}')

    pitchfork_position = 'Near median'
    reclaimed_2sd = False
    if location_mode != "No man's land":
        pf1, pf2 = st.columns([2, 1])
        with pf1:
            pitchfork_position = st.selectbox(
                '5. Pitchfork position',
                [
                    'Below lower 2SD',
                    'Lower 1SD to 2SD',
                    'Lower 0 to 1SD',
                ...
                index=3,
                key=f'sb_pf_{instrument}',
            )
        with pf2:
            if pitchfork_position in ['Below lower 2SD', 'Above upper 2SD']:
                reclaimed_2sd = st.selectbox('Reclaimed back inside 2SD?', ['NO', 'YES'], key=f'sb_reclaim_{instrument}') == 'YES'
            else:
                st.caption('2SD reclaim is only required after an extreme break.')

    submitted = st.button('Get Superbrain Market Verdict', type='primary', use_container_width=True, key=f'sb_submit_{instrument}')

    if submitted:
        verdict = superbrain_market_view(
            location_mode=location_mode,
            macro=macro,
            odme_tilt=odme_tilt,
            exec_of=exec_of,
            aurora=aurora,
            pitchfork_position=pitchfork_position,
            reclaimed_2sd=reclaimed_2sd,
        )
        st.session_state[f'sb_verdict_{instrument}'] = verdict

    verdict = st.session_state.get(f'sb_verdict_{instrument}')
    if verdict:
        decision = verdict['decision']
        if verdict['direction'] == 'BULLISH' and ('ELIGIBLE' in decision or 'LONG' in decision):
            tint = 'tint-green'
        elif verdict['direction'] == 'BEARISH' and ('ELIGIBLE' in decision or 'SHORT' in decision):
            tint = 'tint-red'
        elif decision == 'WAIT':
            tint = 'tint-amber'
        else:
            tint = 'tint-grey'
        render_hero('SUPERBRAIN VERDICT', decision, tint)
        out = st.columns(3)
        with out[0]:
            render_card('Mode', verdict['mode'], location_mode, 'tint-blue')
        with out[1]:
            render_card('Market View', verdict['direction'], 'Deterministic V1', tint)
        with out[2]:
            render_card('ODME Source', odme_dir, odme_tilt, _tint_for_tilt(odme_tilt))
        st.markdown(f"**Why:** {verdict['reason']}")
        st.markdown(f"**Next:** {verdict['next']}")
...
def main_page() -> None:
    ...
    if live_result:
        ...
        render_superbrain_manual(display, instrument)
        st.markdown("---")
